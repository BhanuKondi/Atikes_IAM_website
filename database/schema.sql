...
CREATE TABLE `question` (
  ...
  KEY `ix_question_solution_answer_id` (`solution_answer_id`),
...
CREATE TABLE `saved_question` (
  ...
  `question_id` INT NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_saved_question_user` (`user_id`, `question_id`),
  KEY `ix_saved_question_created_at` (`created_at`),
  KEY `ix_saved_question_user_id` (`user_id`),
  KEY `ix_saved_question_question_id` (`question_id`),
  CONSTRAINT `fk_saved_question_user`
    FOREIGN KEY (`user_id`) REFERENCES `user` (`id`)
    ON DELETE CASCADE,
  CONSTRAINT `fk_saved_question_question`
    FOREIGN KEY (`question_id`) REFERENCES `question` (`id`)
    ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
